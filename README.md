...
- **Real chip mechanics + terminal controls** — pick a denomination ($5/$25/$100/$500/$1,000) off the rail (rendered as clay chips with edge spots), tap a spot to drop chips onto it. The betting controls mirror a real electronic table: **Clear, Undo (removes the last chip), Rebet, and Deal** — no blackjack-style "Double," which isn't a baccarat action — with a live **Total Bet / Balance** readout above them.
- **Live wallet HUD & session record** — the top bar is a game HUD: a tappable wallet showing your bankroll with a running **net P/L** delta (green up / red down) and a bankroll counter that ticks and flashes on every settle. Tapping it opens a **Session** dashboard — a bankroll sparkline chart, the Banker/Player/Tie distribution bar, and tiles for hands, best hand, peak bankroll, current streak and longest run — with a Reset. Your wallet and record **persist across reloads** (localStorage), so you sit back down where you left off.
...
- **Atlantic City high-limit electronic board** — the scoreboard is a recessed, brass-framed cabinet with a glowing **LED result ticker** (the last results as neon pills, newest pulsing) and **LED-lit road markers** on a dark glass display. Five live roads, each with its own authentic icon: Bead Plate solid B/P/T coins, Big Road hollow red/blue rings with green tie slashes and pair dots, and the derived roads shape-coded exactly as on a real board — **Big Eye Boy = hollow ring, Small Road = solid dot, Cockroach Road = diagonal slash** — plotted at their exact grid coordinates with dragon-tail overflow.
- **Plays like an app, not a web page** — a fixed full-height shell with a locked HUD header and betting dock and an internally-scrolling table, lit like a high-limit room (warm brass spotlight, green velvet felt-glow, deep vignette) rather than a flat scrolling document.
...
![Bead Plate, Big Eye Boy, Small Road, Cockroach Road](assets/screenshots/bigroad.png)
![Session dashboard with bankroll sparkline](assets/screenshots/session.png)
